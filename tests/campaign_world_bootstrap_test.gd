...
func _initialize() -> void:
    ...
    check(CampaignBootstrap.from_world(duplicate_realms, characters, dynasties)["state"] == null, "duplicate scenario Realm rejected before indexing")
    var missing_realms: Array[RealmState] = realms.duplicate()
    missing_realms.pop_back()
    check(CampaignBootstrap.from_world(missing_realms, characters, dynasties)["state"] == null, "missing starting Realm rejected")
    var missing_ruler: Array[CharacterState] = characters.duplicate()
    missing_ruler.pop_back()
    check(CampaignBootstrap.from_world(realms, missing_ruler, dynasties)["state"] == null, "missing authored/scenario ruler fails instead of inventing one")
