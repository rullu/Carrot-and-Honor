...
const SHADER: Shader = preload("res://src/presentation/world_map/astra_natural_terrain.gdshader")
const NATURAL_ROOT: String = "res://assets/world_map/astra/natural_world"
const WATER_SHADER: Shader = preload("res://src/presentation/world_map/astra_beauty_water.gdshader")
const PROPS: GDScript = preload("res://src/presentation/world_map/astra_natural_props.gd")
const GAMEPLAY_CLOSE: float = 800.0
const GAMEPLAY_NORMAL: float = 1600.0
const GAMEPLAY_FAR: float = 3600.0
const GAMEPLAY_PITCH: float = 40.0
const VIEW_PRESETS: Array[Dictionary] = [
    {"name":"whole_top_down","target":Vector3.ZERO,"size":29000.0,"top":true},
    {"name":"whole_oblique","target":Vector3.ZERO,"size":30000.0,"top":false},
    {"name":"west_temperate","target":Vector3(-4500,0,-1700),"size":1600.0,"top":false},
    {"name":"southwest_mediterranean","target":Vector3(-12200,0,4600),"size":1200.0,"top":false},
    {"name":"east_desert","target":Vector3(10800,0,4700),"size":1200.0,"top":false},
    {"name":"cold_north","target":Vector3(3500,0,-7900),"size":1800.0,"top":false},
    {"name":"northeast_mountains","target":Vector3(10300,75,-3300),"size":3000.0,"top":false},
    {"name":"central_transition","target":Vector3(7000,0,200),"size":1200.0,"top":false},
    {"name":"lake_parmetos","target":Vector3(-16046,0,2578),"size":1700.0,"top":false},
    {"name":"lake_waring","target":Vector3(12971,0,-4766),"size":1500.0,"top":false},
    {"name":"western_coast","target":Vector3(-5250,0,-1400),"size":1700.0,"top":false},
    {"name":"lowland_detail","target":Vector3(-2700,0,-2300),"size":650.0,"top":false},
    {"name":"desert_detail","target":Vector3(11600,0,4950),"size":650.0,"top":false},
    {"name":"western_region","target":Vector3(-3500,0,-1200),"size":4800.0,"top":false},
    {"name":"eastern_region","target":Vector3(10800,0,4700),"size":6800.0,"top":false},
    {"name":"river_detail","target":Vector3(-2576,0,-928),"size":450.0,"top":false},
]

var _props: Node3D
var _sun: DirectionalLight3D
var _terrain_focus: Camera3D
var _dragging: bool = false
var _development_view: bool = false
var _discovery_index: int = 0
...
func _ready() -> void:
    ...
    var context: Image = Image.load_from_file(ProjectSettings.globalize_path(NATURAL_ROOT.path_join("surface_context_rgba.png")))
    ...
    # Orthographic rendering looks ahead of the real camera's X/Z. Keep the
    # existing clipmap's finest rings at the inspected ground, preserving heights.
    _terrain_focus = Camera3D.new()
    _terrain_focus.name = "TerrainDetailFocus"
    _terrain_focus.far = 100000.0
    add_child(_terrain_focus)
    terrain.set_camera(_terrain_focus)
    camera.current = true
    _set_preset(2)
    ...
    if not _failures.is_empty():
        ...
        get_tree().quit(1)
        return
    print("ASTRA_NATURAL_WORLD_READY: %d regions; %d material sets; runtime shader verified; load %.2fs" % [terrain.data.get_regions_active().size(), albedo.get_layers(), (Time.get_ticks_usec()-_started_usec)/1000000.0])
...
func _unhandled_input(event: InputEvent) -> void:
    ...
    if event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_MIDDLE:
        ...
        var units_per_pixel: float = camera.size/get_viewport().get_visible_rect().size.y
        _camera_target.x -= event.relative.x*units_per_pixel
        _camera_target.z -= event.relative.y*units_per_pixel/(1.0 if _top_down else sin(deg_to_rad(GAMEPLAY_PITCH)))
        _update_camera()
    if event is InputEventMouseButton and event.pressed:
        if event.shift_pressed: _development_view = true
        if event.button_index == MOUSE_BUTTON_WHEEL_UP:
            camera.size = maxf(100.0 if _development_view else GAMEPLAY_CLOSE, camera.size*.82)
            _update_camera()
        elif event.button_index == MOUSE_BUTTON_WHEEL_DOWN:
            camera.size = minf(42000.0 if _development_view else GAMEPLAY_FAR,camera.size*1.22)
            _update_camera()
    elif event is InputEventKey and event.pressed and not event.echo:
        match event.physical_keycode:
            KEY_1, KEY_2, KEY_3, KEY_4, KEY_5, KEY_6, KEY_7, KEY_8, KEY_9:
                _set_preset(2 + int(event.physical_keycode-KEY_1))
            KEY_P:
            ...
            KEY_HOME:
                _development_view = false
                _set_preset(2)
            KEY_0:
                var records: Dictionary = JSON.parse_string(FileAccess.get_file_as_string(NATURAL_ROOT.path_join("natural_districts.json")))
                var point: Array = records.landmarks[_discovery_index%records.landmarks.size()].target
                _discovery_index += 1
                _camera_target = Vector3(point[0],point[1],point[2])
                camera.size = GAMEPLAY_NORMAL
                _top_down = false
                _development_view = false
                _update_camera()
...
func _update_camera() -> void:
    ...
    if _top_down:
        ...
        var pitch: float = deg_to_rad(GAMEPLAY_PITCH)
        camera.position = _camera_target + Vector3(0.0,sin(pitch),cos(pitch))*maxf(camera.size,150.0)*1.1
        camera.look_at(_camera_target,Vector3.UP)
    camera.near = maxf(.5,camera.size*.0002)
    if _terrain_focus != null:
        _terrain_focus.position = _camera_target + Vector3(0,150,0)
    if _sun != null:
        _sun.directional_shadow_max_distance = maxf(900.0,camera.size*2.2)
    if _props != null:
        _props.set_view_size(camera.size)
    view_label.text = "%s | zoom %.0f | 40° gameplay: 800–3600 (default 1600)" % ["TOP-DOWN" if _top_down else "OBLIQUE",camera.size]
...
func _validate_contract(albedo: Texture2DArray, normal_roughness: Texture2DArray) -> void:
    ...
    if not active_code.contains("vec2 hash_position(") or not terrain.material.shader_override_enabled or terrain.material.show_checkered:
...
func _validate_controls() -> void:
    ...
    middle.pressed = true
    _unhandled_input(middle)
    previous_target = _camera_target
    var drag: InputEventMouseMotion = InputEventMouseMotion.new()
    drag.relative = Vector2(40,20)
    _unhandled_input(drag)
    middle.pressed = false
    _unhandled_input(middle)
    if _camera_target.x >= previous_target.x or _camera_target.z >= previous_target.z:
        _failures.append("Middle mouse drag did not pan the ground.")
...
func _capture_validation(output: String) -> void:
    ...
    if "--ground-proof" in OS.get_cmdline_user_args():
        presets.append({"name":"lowland_ground_only","target":Vector3(-2700,0,-2300),"size":650.0,"top":false})
    ...
    for preset: Dictionary in presets:
        ...
        _props.visible = preset.name != "lowland_ground_only"
        ...
        var filename: String = "astra_natural_%s.png" % preset.name
    ...
    var result: Dictionary = {"pass":_failures.is_empty(),"failures":_failures,"views":measurements,
        ...
        "water_shader_sha256":WATER_SHADER.code.sha256_text(),
        "props_shader_sha256":FileAccess.get_sha256("res://src/presentation/world_map/astra_natural_props.gdshader"),
        "natural_manifest_sha256":FileAccess.get_sha256(NATURAL_ROOT.path_join("natural_world_manifest.json")),
        "gpu":RenderingServer.get_video_adapter_name(),
        "vsync_mode":DisplayServer.window_get_vsync_mode(),
        "controls_tested":["V projection","wheel zoom","pan east","middle drag","F frame"],
        "natural_instances":_props.instance_count,"natural_batches":_props.batch_count,
        "camera_tilt_degrees":GAMEPLAY_PITCH,"gameplay_zoom":[GAMEPLAY_CLOSE,GAMEPLAY_NORMAL,GAMEPLAY_FAR],
        "albedo_format":"BC1 / sRGB samples","normal_roughness_format":"BC3 / linear RGBA"}
    var record: FileAccess = FileAccess.open(output.path_join("natural_world_validation.json"),FileAccess.WRITE)
    if record == null:
        _failures.append("Could not write natural_world_validation.json")
    else:
        record.store_string(JSON.stringify(result,"  ")+"\n")
        record.close()
    for failure: String in _failures:
        push_error(failure)
    print("ASTRA_NATURAL_WORLD_VALIDATION_PASS" if _failures.is_empty() else "ASTRA_NATURAL_WORLD_VALIDATION_FAIL")
    get_tree().quit(0 if _failures.is_empty() else 1)


func _set_preset(index: int) -> void:
    var preset: Dictionary = VIEW_PRESETS[clampi(index,0,VIEW_PRESETS.size()-1)]
    _camera_target = preset.target
    _top_down = preset.top
    camera.size = preset.size
    _development_view = index < 2 or camera.size < GAMEPLAY_CLOSE or camera.size > GAMEPLAY_FAR
    _update_camera()


func _bind_natural_world() -> void:
    var textures: Dictionary = {}
    for entry: Array in [["shore_distance","shore_distance.exr",false], ["landscape_character","landscape_character_rgba.png",true],
            ["ecology","ecology_rgba.png",true],["water_features","water_features_rgba.png",false], ["water_optics","water_optics_rgba.png",false],
            ["shore_finish","shore_finish_rgba.png",true],["river_motion","river_motion_rgba.png",false],
            ["water_edge_style","water_edge_style.exr",false]]:
        var source: Image = Image.load_from_file(ProjectSettings.globalize_path(NATURAL_ROOT.path_join(entry[1])))
        if source.get_size() != Vector2i(4096,2304):
            _failures.append("Invalid natural world texture: "+String(entry[1]))
            return
        if entry[2]:
            source.generate_mipmaps()
        var texture: ImageTexture = ImageTexture.create_from_image(source)
        textures[entry[0]] = texture
        _textures.append(texture)
        terrain.material.set_shader_param(StringName(entry[0]),texture)
    var water_material: ShaderMaterial = ShaderMaterial.new()
    water_material.shader = WATER_SHADER
    water_material.set_shader_parameter("wave_spectrum",load("res://src/presentation/world_map/astra_water_spectrum.gd").load_volume())
    for key: String in textures:
        water_material.set_shader_parameter(key,textures[key])
    $SeaLevelReference.material_override = water_material
    $SeaLevelReference.cast_shadow = GeometryInstance3D.SHADOW_CASTING_SETTING_OFF
    var inland_material: ShaderMaterial = water_material.duplicate() as ShaderMaterial
    inland_material.set_shader_parameter("inland",true)
    var raw: PackedFloat32Array = FileAccess.get_file_as_bytes(NATURAL_ROOT.path_join("inland_water_vertices.bin")).to_float32_array()
    ...
    for index: int in range(vertices.size()):
        vertices[index] = Vector3(raw[index*3],raw[index*3+1],raw[index*3+2])
    var arrays: Array = []
    ...
    var inland: MeshInstance3D = MeshInstance3D.new()
    inland.name = "AuthoritativeInlandWater"
    inland.mesh = mesh
    inland.material_override = inland_material
    inland.cast_shadow = GeometryInstance3D.SHADOW_CASTING_SETTING_OFF
    add_child(inland)
    var bank_raw: PackedFloat32Array = FileAccess.get_file_as_bytes(NATURAL_ROOT.path_join("bank_vertices.bin")).to_float32_array()
    var bank_surface: SurfaceTool = SurfaceTool.new()
    bank_surface.begin(Mesh.PRIMITIVE_TRIANGLES)
    for index: int in range(bank_raw.size()/3):
        bank_surface.add_vertex(Vector3(bank_raw[index*3],bank_raw[index*3+1],bank_raw[index*3+2]))
    bank_surface.generate_normals()
    bank_surface.index()
    var bank_material: ShaderMaterial = ShaderMaterial.new()
    bank_material.shader = load("res://src/presentation/world_map/astra_natural_banks.gdshader") as Shader
    bank_material.set_shader_parameter("climate",terrain.material.get_shader_param(&"material_mask"))
    bank_material.set_shader_parameter("shore_finish",textures.shore_finish)
    var banks: MeshInstance3D = MeshInstance3D.new()
    banks.name = "NaturalBankFaces"
    banks.mesh = bank_surface.commit()
    banks.material_override = bank_material
    banks.cast_shadow = GeometryInstance3D.SHADOW_CASTING_SETTING_OFF
    add_child(banks)
    _props = PROPS.new() as Node3D
    _props.name = "NaturalWorldDetails"
    add_child(_props)
    _props.build()
    var reactions: Node3D = load("res://src/presentation/world_map/astra_water_reactions.gd").new() as Node3D
    reactions.name = "SelectedWaterReactions"
    add_child(reactions)
    reactions.build(textures)
    _add_clouds()


func _add_clouds() -> void:
    var clouds: Node3D = load("res://src/presentation/world_map/astra_clouds.gd").new() as Node3D
    clouds.name = "MovingCloudVolumes"
    add_child(clouds)
    clouds.build()
